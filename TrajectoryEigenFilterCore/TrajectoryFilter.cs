...
public sealed class TrajectoryFilter
{
    ...
    // newest boundary. Each lookahead selects a hard retained-mode count from
    // the reconstruction-accuracy schedule. Estimates are weighted by
    // inverse unit-white-noise transmission.  R=0 is intentionally just the
    // zero-lookahead estimator; there are no negative-delay terms.
    public bool PushAdaptiveDelayAverage(Vector2 point, int lag, out FilteredWindow result)
    {
        ...
        if (adaptiveLag != lag)
        {
            ...
            for (int r = 0; r <= lag; r++)
            {
                int modes = Model.AdaptiveModesForLookaheadSamples(r);
                var built = Model.BuildModeCountKernel(Model.N - 1 - r, modes);
    ...
    static double[] Solve3(double[,] a, double[] b)
    {
        ...
        var z = new double[3]; for (int i = 2; i >= 0; i--) { double q = v[i]; for (int j = i + 1; j < 3; j++) q -= m[i, j] * z[j]; z[i] = q / m[i, i]; }
        return z;
    }
...
public sealed record FilteredWindow(double[] X, double[] Y, IReadOnlyList<OutlierReplacement> Replacements, double Complexity, int? TargetIndex = null)
{
    ...
    public Vector2 At(double index)
    {
        ...
        if (index >= n - 1)
        {
            ...
            return new((float)(X[n - 1] + dx * t), (float)(Y[n - 1] + dy * t));
        }
