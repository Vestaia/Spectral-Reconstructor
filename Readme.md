...
| Resampling frequency | $f_o$ | 1000 Hz | Sets the scheduled output rate when async mode is enabled. Use 125, 250, 500, or 1000 Hz as required by the selected OTD scheduling mode. |
| Enable async mode | — | True | Emits reconstructed positions at the configured resampling frequency. When disabled, the filter emits one reconstructed output for each tablet report. |
